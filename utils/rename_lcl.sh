...
find . \( -name "*.pp" -o -name "*.lfm" -o -name "*.ttd" -o -name "*.pas" -o -name "*.lpr" -o -name "*.mysql" -o -name "*.sql" \) -exec str_replace $1 $2 {} \;
find . \( -name "*.po" -o -name "*.lrt" \) -exec str_replace $SEARCH_UC $REPLACE_UC {} \;
find . -name "*.po" -exec str_replace $SEARCH_LC $REPLACE_LC {} \;
