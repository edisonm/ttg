...
{$IFDEF SPANISH}
  sAbout = 'Acerca de';
{$IFDEF WIN32}
  sFreeMemoryLabel = 'Memoria física:';
  sFreeResourcesLabel = 'Memoria en uso:';
{$ELSE}
  sFreeMemoryLabel = 'Memoria libre:';
  sFreeResourcesLabel = 'Recursos del sistema:';
{$ENDIF WIN32}
  sVersion = 'Versión %d.%.2d';
  sFileVer = 'Versión %s';
  sExeFile = 'Archivo Ejecutable %s';
{$ELSE}
  sAbout = 'About';
{$IFDEF WIN32}
...
{$IFDEF SPANISH}
  DefVersion = '0.09';
  SCopyright = 'Derechos reservados © %d';
{$ELSE}
  DefVersion = '0.09';
  ...
  try
    ...
    try
      if VerInfo.Valid then begin
        FYearLabel.Caption := VerInfo.LegalCopyright;
...
{$ENDIF}
      ...
      FCopyright.Caption := Format(sExeFile,
        [FormatDateTime('c', VerInfo.VerFileDate)]);
